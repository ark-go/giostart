...
func Create(leftProcent int, barWidth int) *Split {
	if barWidth < 0 {
		barWidth = 0
	}
	s := &Split{
...
func (s *Split) LeftPanelWidth(p int) {
	if !s.initVal {
		if p > 0 && p <= 100 {
...
func (s *Split) Layout(gtx layout.Context, left, right layout.Widget) layout.Dimensions {
	...
	leftsize := int(proportion*float32(gtx.Constraints.Max.X) - float32(gtx.Px(s.Bar)))
...

	{
		...
		barRect := image.Rect(leftsize, 0, rightoffset, gtxN.Constraints.Max.Y)
		// создадим clip чтоб закрасить цветом
		clip.Rect{Min: barRect.Min, Max: barRect.Max}.Add(gtx.Ops)
...

	{
		if rightoffset-leftsize > 4 {
			stack := op.Save(gtx.Ops)
			// украшаем
			barRect := image.Rect(leftsize+2, 0, rightoffset-2, gtxN.Constraints.Max.Y)
			// создадим clip чтоб закрасить цветом
			clip.Rect{Min: barRect.Min, Max: barRect.Max}.Add(gtx.Ops)
...

	{
		if s.BarKnobLedg > 0 {
			...
			stack := op.Save(gtx.Ops)
			// test рисуем сверху еще чтото
			p10 := gtxN.Constraints.Max.Y * 10 / 100        // высота ручки 10 %
			t10 := (gtxN.Constraints.Max.Y / 2) - (p10 / 2) // начало отрисовки
			//barRect = image.Rect(leftsize-2, 0, rightoffset+2, gtx.Constraints.Max.Y)
			// ширина утолщения
			...
			he := barRect.Max.Y - barRect.Min.Y
			if he < widths {
				radius = float32(he / 2)
			}
			// создадим clip чтоб закрасить цветом
